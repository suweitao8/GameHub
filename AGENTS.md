...
- 本项目网页（本地 `127.0.0.1:9000` 与线上站点）的运行、交互调试、截图和 UI 验收，一律使用 ZCode 内置浏览器（browser-use control-browser 技能）。其导航、DOM 快照读取、点击/输入交互、截图、页面内 JS 执行能力已验证可覆盖全部开发需求，默认不因"内置浏览器做不到"而换工具。
- 禁止使用桌面控制工具（computer-use/CUA）驱动用户本机 Chrome/Edge 等真实浏览器做本项目网页调试与验收；同样禁止 Chrome CDP/控制工具和独立浏览器工具。内置浏览器不可用时报告阻塞，不得自动切换。
- 内置浏览器已知怪癖：SPA 路由跳转后标签页偶发回到 `about:blank`，重新 `goto` 目标 URL 即可恢复，不构成切换工具的理由。
- 例外：需要用户登录态的外部控制台（如腾讯云控制台）操作不属于本项目网页调试，可使用桌面控制。

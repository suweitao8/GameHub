...
优化登录用户头像资料卡的单头像展开转场，修复重复头像、桌面端定位不一致和原位残留圆框问题，详见[完整更新记录](docs/releases/release-notes.md)。

统一游戏中心页头、移动菜单、浏览器标签页与服务器默认 Logo 为同一枚彩虹渐变 G 标识，详见[完整更新记录](docs/releases/release-notes.md)。

...
**Architecture:** 保留现有 Angular Header 的 popup signal、延迟关闭和点击路由，只在模板给头像容器增加由 `isOpenPopover('avatar')` 驱动的打开态 class。唯一 `.game-user-avatar` 继续作为按钮子节点，通过 CSS `transform` 跨过 Header 与资料卡间隙；资料卡只提供背景、名称、硬币和退出操作，并用顶部留白承接转场后的头像。静态契约先锁定单头像、打开态几何、移动端退化和 reduced-motion，再用真实浏览器验证实际矩形位置与可操作性。
...
- `client/src/app/header/header.component.scss`：头像 transform 转场、层级、资料卡顶部留白、移动端关闭空间转场、reduced-motion 规则。
...
assert(
  submitHeaderScss.includes('.game-avatar-menu-open > .tertiary-button .game-user-avatar') &&
    submitHeaderScss.includes('transform: translate3d(0, calc(var(--header-height) / 2 + 0.5rem), 0) scale(2.12);') &&
...
assert(
  ...
  'GameHub avatar transition must keep the transformed image above the card and disable cross-header motion on mobile'
)
```

- [ ] **Step 2: 运行契约确认它因缺少目标行为而失败**
...
- [ ] **Step 1: 让容器 class 由现有 popup 状态驱动，并删除重复图片**
...
:host-context(.game-experience) .logged-in-container.game-avatar-menu-open > .tertiary-button .game-user-avatar {
  ...
  transform: translate3d(0, calc(var(--header-height) / 2 + 0.5rem), 0) scale(2.12);
}
```

资料卡自身继续使用 `z-index: 60`，`.game-avatar-profile` 改为 `padding: 4.25rem 1.1rem 1rem`，给转场头像和名称之间留下稳定空间。

- [ ] **Step 4: 明确移动端和 reduced-motion 行为**
...
- [ ] **Step 3: 检查进入卡片、关闭回位和键盘路径**

从放大头像滑入 `.game-avatar-hover-card`，点击“退出登录”验证卡片仍可操作；重新登录后将鼠标移出 wrapper，等待 950ms，确认头像回到约 `34x34` 的原位且卡片卸载。用 Tab 聚焦头像，确认 `aria-expanded="true"`、资料卡挂载、focus ring 可见，并在焦点移出后收回。

- [ ] **Step 4: 检查移动端和 reduced-motion**

在 375px 宽度下确认页面无横向溢出、头像点击仍进入原有个人主页路由，且不存在跨 Header 的 transform。开启 `prefers-reduced-motion: reduce` 后确认资料卡打开时头像直接处于最终中心位置、transition 为 `none`，没有动画闪烁。
...
Expected: 只包含 Header 模板/SCSS、契约脚本、README/release notes 和已提交的 spec/plan；不包含 `node_modules`、`dist`、截图、日志或本地配置。

- [ ] **Step 2: 请求代码复核**

复核重点：唯一头像是否真的由现有 popup 状态控制；transform 是否把头像中心准确放到卡片中心；放大头像是否遮挡退出按钮；pointer/focus 延迟关闭是否保留；移动端和 reduced-motion 是否完整；是否重新引入重复 `img`。高优先级问题处理后重新运行 Task 2-4 的定向验证。

- [ ] **Step 3: 提交实现**

```powershell
git add client/src/app/header/header.component.html client/src/app/header/header.component.scss scripts/verify-gamehub-client.mjs README.md docs/releases/release-notes.md

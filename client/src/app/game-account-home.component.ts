      ...
      <div class="game-community-content">
        ...
        @if (user; as currentUser) {
          <section class="game-account-profile">
            ...
            <a [routerLink]="['/games/author', currentUser.account?.id]">
              查看我的主页 →
            </a>
          </section>
          ...
          <p class="game-account-note">账号基础设置正在迁移到 GameHub，当前登录、主页、创作和社区功能均可正常使用。</p>
        } @else {
          <section class="game-account-empty"><h2>请先登录</h2><p>登录后即可管理你的游戏和社区互动。</p><a routerLink="/login">去登录</a></section>
        }
      </div>
    </main>
  `,
